...
class VideoActivity : AppCompatActivity() { // КАК У ИГОРЯ - разрешение на пользование камерой
    ...
    private fun startCamera() { // КАК ИГОРЯ - начало работы с камерой
        val preview = Preview.Builder().build().also {
            it.setSurfaceProvider(binding.viewFinder.surfaceProvider)
        }
        cameraProviderFuture.addListener({
            val cameraProvider = cameraProviderFuture.get()
            val recorder = Recorder.Builder() // для начала записи видео
